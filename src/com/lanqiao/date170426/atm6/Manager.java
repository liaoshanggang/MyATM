...
public class Manager {
	 ...
	 * private final Lock readLock = wrlock.readLock(); private final Lock
	 * writerLock = wrlock.writeLock();
	 */
	...
	private void transferMenu() {
		...
		Account a = u.getUser(cardNo).getAccount();// 当前的账户
		Account b = u.getUser(id).getAccount();// 转到其他账户
	...
	public synchronized void tranMoney(Account a, Account b, double tranMoney) {
		...
		try {
			...
			System.out.println(Thread.currentThread() + ":转账金额" + tranMoney
					+ ",从 帐号" + a.getCardNo() + " 到帐号 " + b.getCardNo());
	...
	public double getTotalBalance() {
		System.out.println("每次从文件转账更新的对应账户的数据");
		UserDao u = UserDao.getUserDao();
		double total = 0;
